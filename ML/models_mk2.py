"""
Модели MK2 - прогноз выкупа в момент передачи заказа в доставку
Logistic Regression, Random Forest, XGBoost, CatBoost
All features from MK1 + logistics timing features
"""
...
model_path = ml_path / 'Models_mk2'
log_path = ml_path / 'Logs_mk2'
img_path = ml_path / 'Images_mk2'
...
drop2 = [
    ...
    # модель 2: информация ПОСЛЕ передачи в доставку
...
def drawCoef(model, features, save_path):
    ...
    plt.title('Feature Coefficients - LogReg-MK2')
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
    print(f'График сохранен: {save_path.name}')
...
def drawGroupedCoef(model, features, save_path):
    ...
    ax.set_title('Важность групп признаков - LogReg-MK2')
    ax.set_xlim(0, group_series.max() * 1.15)
    for bar, val in zip(bars, group_series.values):
        ax.text(val + group_series.max() * 0.01, bar.get_y() + bar.get_height() / 2,
...
drawCoef(lr_model, features, img_path / 'LogReg-MK2_coef.png')
drawGroupedCoef(lr_model, features, img_path / 'LogReg-MK2_grouped.png')

rf_perimp = permutation_importance(rf_model, X_test, y_test, n_repeats=10, random_state=seed, scoring='roc_auc')
drawTopFeatures(pd.Series(rf_perimp.importances_mean, index=features), 'RF-MK2', img_path / 'RF-MK2_perimp.png')
drawGroupedImp(pd.Series(rf_perimp.importances_mean, index=features), 'RF-MK2', img_path / 'RF-MK2_grouped.png')

xgb_imp = pd.Series(xgb_model.feature_importances_, index=features)
drawTopFeatures(xgb_imp, 'XGBoost-MK2', img_path / 'XGBoost-MK2_features.png')
drawGroupedImp(xgb_imp, 'XGBoost-MK2', img_path / 'XGBoost-MK2_grouped.png')

cb_imp = pd.Series(cb_model.feature_importances_, index=features)
drawTopFeatures(cb_imp, 'CatBoost-MK2', img_path / 'CatBoost-MK2_features.png')
drawGroupedImp(cb_imp, 'CatBoost-MK2', img_path / 'CatBoost-MK2_grouped.png')
...
with open(model_path / 'LogReg-MK2.pkl', 'wb') as f:
    pickle.dump({'model': lr_model, 'scaler': scaler, 'features': features}, f)
print('\nМодель сохранена: LogReg-MK2.pkl')

with open(model_path / 'RF-MK2.pkl', 'wb') as f:
    pickle.dump({'model': rf_model, 'features': features}, f)
print('Модель сохранена: RF-MK2.pkl')

xgb_model.save_model(str(model_path / 'XGBoost-MK2.json'))
print('Модель сохранена: XGBoost-MK2.json')

cb_model.save_model(str(model_path / 'CatBoost-MK2.cbm'))
print('Модель сохранена: CatBoost-MK2.cbm')
...
new_rows = pd.DataFrame([
    {'Model': 'LogReg-MK2',   'bestTime': lr_time,  'totalTime': lr_time,  'AUROCIndex': round(lr_auc, 4),  'F1Index': round(lr_f1, 4)},
    {'Model': 'RF-MK2',       'bestTime': rf_time,  'totalTime': rf_time,  'AUROCIndex': round(rf_auc, 4),  'F1Index': round(rf_f1, 4)},
    {'Model': 'XGBoost-MK2',  'bestTime': xgb_time, 'totalTime': xgb_time, 'AUROCIndex': round(xgb_auc, 4), 'F1Index': round(xgb_f1, 4)},
    {'Model': 'CatBoost-MK2', 'bestTime': cb_time,  'totalTime': cb_time,  'AUROCIndex': round(cb_auc, 4),  'F1Index': round(cb_f1, 4)},
])
...
if not records_file.exists():
    ...
    other = existing[~existing['Model'].isin(['LogReg-MK2', 'RF-MK2', 'XGBoost-MK2', 'CatBoost-MK2'])]
